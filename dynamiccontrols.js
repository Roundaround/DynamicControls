...
//#region Utility jQuery functions and clone fix.
...
$.fn.dynamicList = function (args) {
    ...
    var i, j;
    for (i = 0; i < 100; i++) {
        if ($('[data-dlid="' + i + '"]').length === 0) {
            id = i;
            break;
        }
    }

    Ω.defaultData = args.defaultData || 'Enter Data Here.';
    Ω.data = args.data || [Ω.defaultData, Ω.defaultData, Ω.defaultData, Ω.defaultData];
    var overwrite = args.overwrite === undefined ? true : args.overwrite;
    Ω.highlightColor = args.highlightColor || 'rgba(220, 220, 0, 0.4)';
    ...
    function insertAfter() {
        ...
        var input = $(document.createElement('input')).attr('type', 'text').appendTo(liWrapper);
        input.val(Ω.defaultData);
        input.attr('placeholder', Ω.defaultData);
        registerEvents('control', input);
    ...
    function registerEvents(type, obj) {
        ...
        if (type == 'control') {
            ...
            obj.focus(function (e) {
                Ω.css('background-color', 'rgb(245, 245, 245)');
            });
            obj.blur(function (e) {
    ...
    var style = $([
        ...
        '        background-color: ' + Ω.highlightColor + ';',
        '    }',
...
$.fn.dynamicTable = function (args) {
    ...
    function data(i, j) {
        ...
        if (arguments.length == 1) return Ω.data[i];
        return Ω.data;
    }
    ...
    function normalizeData() {
        for (var i = 0; i < rows() ; i++) {
            if (!isArray(data(i))) {
                Ω.data[i] = [].repeat(Ω.defaultData, cols());
            } else if (data(i).length != cols()) {
                for (var j = data(i).length; j < cols() ; j++) {
                    data(i).push(Ω.defaultData);
                }
            }
        }
    }
